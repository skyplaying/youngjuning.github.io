...
- significant `[sɪɡˈnɪfɪkənt]`
  ...
  - n. 象征；有意义的事物
- Spawn
  > 在计算机领域中，Spawn 是指加载并执行新的子进程的函数。当前进程可以等待子进程终止，也可以继续执行并发计算。创建新的子进程需要足够的内存，子进程和当前程序都可以在其中执行。
  >
  > DOS 中有一系列 Spawn 函数，由 Microsoft Windows 继承。
  >
  > 在 POSIX 标准的可选扩展中，还有一组不同的 spawn 函数。
